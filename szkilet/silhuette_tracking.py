...
class HumanTracker:
    def __init__(self, camera_index=0, model_path="yolov8n.pt"):
        """
        Klasa do śledzenia pozycji człowieka za pomocą YOLO w tle.
        """
        self.model = YOLO(model_path)  # Ładowanie modelu YOLO
        self.camera_index = camera_index
        self.running = False
        self.thread = None
        self.offset = 0  # Przesunięcie sylwetki względem centrum
        self.direction = "Sylwetka w centrum"  # Kierunek korekty
        ...
        while self.running:
            ...
            for result in results[0].boxes:
                ...
                if cls == 0 and confidence > 0.75:  # Detekcja człowieka
                    ...
                    self.direction = (
                        f"Skręć w lewo -{abs(self.offset)}px"
                        if self.offset < 0
                        else f"Skręć w prawo +{abs(self.offset)}px"
                        if self.offset > 0
                        else "Sylwetka w centrum"
                    )
                    break
